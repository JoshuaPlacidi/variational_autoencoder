...
#
# ResNet VAE taken from https://github.com/LukeDitria/CNN-VAE
#
...
class VAE(nn.Module):
    ...
    """
    def __init__(self, channel_in=3, ch=64, latent_channels=64):
        super(VAE, self).__init__()
        """
        ResNet VAE Network

        channel_in:
            number of channels of the image, e.g. 3 for rgb
        ch:
            size of intermediate representations, larger means more trainable parameters
        z:
            the number of channels of the latent representation
...
if __name__ == "__main__":
    ...
    model = VAE(channel_in=3, ch=64, latent_channels=512)

    rand_input = torch.randn((1,3,64,64))
    latents, _, _ = model.encoder(rand_input)
    recon_image = model.decoder(latents)
